...
function Initialisation_Run()
    ...
    sys.magnet = 0.32; %T - 320mT
    % SPins to be viewed.
    sys.isotopes = {'E4'}; %Number following defines multiplicity, not #of Electrons. https://spindynamics.org/wiki/index.php?title=Spin_system_specification - isotopes
    sys.disable={'trajlevel'}; % (For less than five spins, speeds up computation.)
    %           SPIN INTERACTIONS
    % defines structure of spins. 
    inter.coupling.matrix{1,1}=[4 0 0; 0 4 0; 0 0 -8]*10^6;
    ...
    inter.temperature=273; %Kelvin. Default: Spinach uses the high-temperature approximation. 
...
%gaussleg.m for magic stuff to do with linewidth calculating.
    ...
    parameters.sweep = 2e8;     % Width of Sweep for First Figure.
    % Linewidth?
    parameters.fwhm=0.01;    %line FWHM, Tesla

    parameters.spins = {'E4'}; % spins of the molecule that will be activlley operated on.
    ...
    % Pulse duration
    parameters.pulse_dur=[1e-9 2e-9 2e-9];       % altered 30ns suggestion - second spin takes slightly longer.
    ...
    parameters.pulse_frq=[10.1034*10e6 16.1034*10e6 16.1034*10e6];
    %        GAPS BETWEEN PULSES
    parameters.p1_p3_gap=1e-6;
    %fidelity of p2 
    parameters.p2_nsteps=100;
    parameters.DeerCut=0; %ammount of the Deer data to cut the echo stack, as a decimal from 0-1. this is dedeucted from both ends. 
